...
(function () {
    ...
    function init() {
        ...
        // CANVAS + WebGL SETUP
        // =========================================================================
        let canvas = document.getElementById('ripple-canvas');
        if (!canvas) {
            ...
            const overlay = document.getElementById('opening-overlay');
            if (overlay) {
                const textContent = overlay.querySelector('.opening-content');
                if (textContent) {
                    overlay.insertBefore(canvas, textContent);
                } else {
    ...
    `;
        ...
        function animate(now) {
            ...
            if (!textTriggered && elapsed >= TEXT_DELAY) {
                textTriggered = true;
                const overlayContent = document.querySelector('.opening-content');
                if (overlayContent) overlayContent.classList.add('active');
            }

            if (!subtitleTriggered && elapsed >= SUBTITLE_DELAY) {
                subtitleTriggered = true;
                const fadeEl = document.getElementById('fade-text') || document.querySelector('.fade-text');
                if (fadeEl) fadeEl.classList.add('active'); // Still adding active here for individual control if needed later
            }
            ...
            // Fade out overlay eventually unless it's a test page
            if (!window.isOpeningTest && elapsed > 6.0) {
                const overlay = document.getElementById('opening-overlay');
                if (overlay) {
