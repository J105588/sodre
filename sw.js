const CACHE_NAME = "sodre-cache-v1.4.0"; // Increment version
...
const ICON_URL =
    "https://lh3.googleusercontent.com/a/ACg8ocKrevxxn-jyPFTJ3zy5r6EFRGmv0Tp8-qWyb3bMaXduuMzHS0Y=s400-c";
...
self.addEventListener("fetch", (event) => {
    ...
    const isSupabaseStorage =
        ENV.SUPABASE_URL &&
        url.href.startsWith(ENV.SUPABASE_URL) &&
    ...
    if (isSupabaseStorage || isSodreData) {
        ...
        // Exclude ZIP, PDF, DOC, audio, video etc. from opaque caching because we need to read them as Blobs for Web Share or native playback
        const isNonImageFile =
            /\.(zip|rar|pdf|doc|docx|xls|xlsx|ppt|pptx|txt|csv|mp3|wav|ogg|m4a|mp4|mov|avi|webm)(\?.*)?$/i.test(
                url.href,
            );

        if (isNonImageFile) {
            // For downloads, bypass SW cache to ensure we get a normal CORS response (or full network response)
            // that can be transformed into a Blob on the frontend.
            return;
        }
